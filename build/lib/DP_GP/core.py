...
class gibbs_sampler():
    ...
    '''
    def __init__(self, gene_expression_matrix, t, gene_names, max_num_iterations, max_iters, optimizer, burnIn_phaseI, burnIn_phaseII, alpha, m, s, check_convergence, sigma_n_init, sigma_n2_shape, sigma_n2_rate):
            ...
            if clusterID in self.last_MVN_by_cluster_by_gene and gene in self.last_MVN_by_cluster_by_gene[clusterID]:
                ...
                if (self.last_cluster[gene] == clusterID): # if gene's last belonged to this cluster under consideration
                    ...
                    if singleton : 
                        ...
                        lik[index] = scipy.stats.multivariate_normal.logpdf(self.gene_expression_matrix[gene,:], np.hstack(self.clusters[clusterID].mean), self.clusters[clusterID].K + noise)
                    else:
                        lik[index] = scipy.stats.multivariate_normal.logpdf(self.gene_expression_matrix[gene,:], np.hstack(self.clusters[clusterID].mean), self.clusters[clusterID].covK)
