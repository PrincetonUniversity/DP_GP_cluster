...
def plot_cluster_gene_expression(clusters, gene_expression_matrix, t, time_unit, output_path_prefix, plot_types):
    ...
    :param clusters: dictionary of CLUSTER objects
    :type clusters: dict
    ...
    for c, IDs in enumerate(IDs_split):
        ...
        for i, ID in enumerate(IDs):
            ...
            Xgrid = np.vstack(np.linspace(min(t), max(t), num=300))
            # calculate mean at grid of x values
            m, v = clusters[ID].model.predict(Xgrid, full_cov=False)
            m = np.vstack(m.mean(axis=1))
            # calculate variance at grid of x values
            lower, upper = clusters[ID].model.predict_quantiles(Xgrid)
            # plot mean and variance ribbon
            GPy.plotting.matplot_dep.base_plots.gpplot(Xgrid, m, lower[:, 0], upper[:, 0], ax=ax)
            ax.set_xlim((min(t),max(t)))
            ax.set_ylim((-3,3))
            # plot an x-axis at zero
            plt.axhline(0, color='black', ls='--', alpha=0.5)
            # plot the expression of each gene in the cluster
            for gene in clusters[ID].members:
                ax.plot(t, np.array(gene_expression_matrix.ix[gene]), color='red', alpha=0.25)
...
#     plt.axvline(burnIn_phaseI, alpha = 0.5, color = 'g', label = "Burn-in phase I ends")
